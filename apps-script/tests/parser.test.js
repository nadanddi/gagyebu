...
assert.equal(context.merchantKey_('지에스 25 전북대점'), 'GS25');
assert.equal(context.merchantKey_('GS25 서신점'), 'GS25');
assert.equal(
  context.matchSimpleRule_('GS25 전북대점', [{key: 'GS25', example: 'GS25 전북대점', category: '생활용품'}]).category,
  '생활용품'
);
...
const simpleImported = context.simpleLedgerRow_({
  ...
  bucket: '지출', note: ''
}, [{key: 'GS25', example: 'GS25 전북대점', category: '생활용품'}]);
